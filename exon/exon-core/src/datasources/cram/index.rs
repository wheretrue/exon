...
pub(crate) struct CRAMIndexRecordChunk {
    records: Vec<noodles::cram::crai::Record>,
}

impl From<Vec<noodles::cram::crai::Record>> for CRAMIndexRecordChunk {
    fn from(records: Vec<noodles::cram::crai::Record>) -> Self {
        Self { records }
    }
}
...
pub(crate) async fn augment_file_with_crai_record_chunks(
    ...
    let chunks = index_records
            ...
            if let Some(seq_id) = header.reference_sequences().get_index_of(region.name()) {
                if let Some(r_seq_id) = r.reference_sequence_id() {
                    ...
                    if let Some(start) = r.alignment_start() {
                        return region.interval().contains(start);
                    } else {
                        return false;
                    }
                } else {
        ...
        .into_iter()
        .map(|records| {
            let record_chunk = CRAMIndexRecordChunk::from(records.cloned().collect::<Vec<_>>());

            let mut pf = partitioned_file.clone();
            pf.extensions = Some(Arc::new(record_chunk));
