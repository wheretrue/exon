...
use super::table_provider::{BAMListingTableConfig, ListingBAMTable, ListingBAMTableOptions};

/// A `ListingTableFactory` that adapts Exon FileFormats to `TableProvider`s.
#[derive(Debug, Clone, Default)]
pub struct BAMTableProviderFactory {}

#[async_trait]
impl TableProviderFactory for BAMTableProviderFactory {
    async fn create(
        &self,
        state: &SessionState,
        ...
        let options = ListingBAMTableOptions::new(file_compression_type);

        let table_path = ListingTableUrl::parse(&cmd.location)?;
        let schema = options.infer_schema(state, &table_path).await?;

        let config = BAMListingTableConfig::new(table_path).with_options(options);
