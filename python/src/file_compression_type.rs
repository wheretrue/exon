...
use crate::error::{ExonPyError, ExonPyResult};
...
impl FileCompressionType {
    #[new]
    fn from_str(compression_string: &str) -> ExonPyResult<Self> {
        let inner = DFFileCompressionType::from_str(compression_string)
            .map_err(|e| ExonPyError::new(&format!("Invalid compression type: {}", e)))?;
...
impl From<FileCompressionType> for DFFileCompressionType {
    fn from(compression_type: FileCompressionType) -> DFFileCompressionType {
        compression_type.inner
    }
}
