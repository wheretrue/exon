...
use crate::error::{self, ExonPyError};
use crate::execution_results::PyExecutionResult;
...
impl ExonSessionContext {
    ...
    fn read_fastq_file(
        ...
        let df = wait_for_future(py, result).map_err(error::ExonPyError::from)?;
    ...
    fn execute(&mut self, query: &str, py: Python) -> PyResult<()> {
        ...
        wait_for_future(py, df.collect()).map_err(error::ExonPyError::from)?;
    ...
    fn register_object_store_from_url(&mut self, url: &str, py: Python) -> PyResult<()> {
        ...
        wait_for_future(py, registration).map_err(ExonPyError::from)?;
