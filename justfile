...
run-benchmarks:
	...
	cargo build --release --package exon-benchmarks
	...
	hyperfine --runs 5 --export-json exon-benchmarks/results/fasta-meth-scan_{{GIT_SHA}}.json \
		-n exon-gzip \
		'./target/release/exon-benchmarks fasta-codon-scan -p ./exon-benchmarks/data/uniprot_sprot.fasta.gz -c gzip' \
		-n exon-zstd \
		'./target/release/exon-benchmarks fasta-codon-scan -p ./exon-benchmarks/data/uniprot_sprot.fasta.zst -c zstd' \
		-n exon-no-compression \
		'./target/release/exon-benchmarks fasta-codon-scan -p ./exon-benchmarks/data/uniprot_sprot.fasta' \
		-n biopython-gzip \
