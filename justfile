...
run-benchmarks:
	...
	# Run vcf benchmarks.
	hyperfine --runs 2 --export-json benchmarks/results/vcf-query-{{GIT_SHA}}.json \
		-n bcftools \
	...
	# Run bam benchmarks.
	hyperfine --runs 2 --export-json benchmarks/results/bam-query-{{GIT_SHA}}.json \
		-n samtools \
	...
	hyperfine --runs 5 --export-json benchmarks/results/fasta-meth-scan-{{GIT_SHA}}.json \
		-n gzip \
